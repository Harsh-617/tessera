    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f9fafb' }}>
      ...
      <main style={{ flex: 1, padding: '40px 48px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 32 }}>
          ...
          <div>
            <h1 style={{ fontSize: 20, fontWeight: 800 }}>{link.mentor?.full_name}</h1>
            <div style={{ fontSize: 13, color: '#6b7280', marginBottom: 6 }}>{link.mentor?.job_title} · {link.mentor?.current_company}</div>
        ...
        {tab === 'Activity' && (
          <div>
            <form onSubmit={submitCheckin} style={{ background: '#fff', borderRadius: 12, padding: 24, marginBottom: 20, boxShadow: '0 1px 6px rgba(0,0,0,0.06)' }}>
              ...
              <button type="submit" style={{ padding: '9px 24px', borderRadius: 8, background: '#0ea5e9', color: '#fff', border: 'none', fontWeight: 600, cursor: 'pointer' }}>Log</button>
            </form>
            {checkins.map(c => (
